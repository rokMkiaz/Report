...
1. 메모리 압박의 경우 자주 발생하는 것으로 보인다. 처음 셋팅 이후, 프로시저 호출지연으로 인한 인덱싱 작업들이 주요 원인인거 같다. 비클러스터형으로 작업할 경우 메모리를 더 사용하고 재정렬된 정보를 할당해제 하지 않는다. 그렇기에 인덱스 재작업 후 점검시 DB를 종료했다 켜주는게 좋을 것으로 보인다.
