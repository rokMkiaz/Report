...
- 고유효과, 품질 부여, 속성변경, 명중/회피 스탯 부여 등 고유 효과를 랜덤하게 넣어주는 부분 문서화 및 개별 클래스작업. 기존 하드코딩됨 + 문서화가 안되어져 있었고, 클래스가 명확하지 않아 Item 내부 로직에서 변수를 사용하고 있었음. 부여하는 매니저를 따로 생성하였으며 여러 옵션이 생겨도 추가하기 쉽게 모듈화를 진행하였음.
...
- 랭킹시스템 변경, 기존 레벨을 길드탭에서만 띄움 + 길드 공헌도에 대한 랭킹 시스템 추가
- 기존 추천 방파 시스템에 엮여있던 랭킹을 보여주던 기능을 완전 분리하였음. 업데이트도 따로 DB 보다는 캐싱을 해서 동적으로 바뀌는 방식으로 바꿈.
...
- 대각선 가중치 잘못 적용을 정상적으로 14주게 변경
- 실패시 길찾기 실패카운트를 초기화 안하던 부분 수정
<details> 
...
#### 11-2. 경험치 분배 시스템
기존 막타 독식체제에서 분배시스템으로 변경, 여러 유저가 공격배율에 따라 경험치,돈을 나눠 먹는다. 업적같은 경우는 공격한 유저들에 대해서 전부 오르게 했으며 기존 파티시스템의 경험치 분배 방식도 바뀐 방식에 맞게 변경하였다.
- 기존 잡은 유저를 기준으로 경험치 분배 할 경우 여러 가정을 해야할게 많아져, 몬스터의 위치를 기준으로 일정범위 내에 있는 유저들에 한해서 경험치,동전을 분배하게 하였다.

#### 11-3. 객체 강도 시스템
생성될때 몬스터의 스케일을 미묘하게 다르게 하고, 해당 몬스터의 체력, 경험치, 공격력이 다르게 해달라는 요청이 있어 개발하였다.

### 12. 수련장 시스템 변경
- 기존 경험치 부여 부분을 NPC타입을 통한 하드코딩이 아닌 NPC가 수련장 포인터를 들고서 따로 부여하게 변경
- 수련장 전체적인 NPC관리를 할 수 있도록 변경.
- 디폴트 셋팅 관련해서 쉽게 설정할 수 있게 변경
- 기존 수련장에 투자하는 금액이 안쌓이며 시스템 컨슘이 이루어지게 하여 인플레를 해소시키는 용도로 변경하였다.
